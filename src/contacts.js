$(document).ready(function(){
								...
								$.ajax({
									...
									success:function(data){
										...
										$.each(data["nameres"],function(key,value){
											...
											var rows = '<td><img src="images/icon.png" class="img-circle" alt="contact img" width="70" height="70"/></td><td><p><b>'+fname+'</p><p>'+mname+'</p><p>'+lname+'</p></td>';
											$("#printname").append(rows);
										});

										$.each(data["addressres"],function(key,value)
										 {
										 	// alert(data["addressres"].length);
										 	...
										 	//var rows = '<td>Address</td><td><table><tbody><th>'+addresstype+'</th><tr><td>'+address+'</td><td>'+city+'</td><td>'+state+'</td><td>'+zip+'</td></tr></tbody></table></td>';
										 	if(address=="")
										 	{
										 		address = 'N/A';
										 	}
										 	...
										 	//var rows = '<table><tbody><th>'+addresstype+'</th><tr><td>'+address+',</td><td>'+city+',</td><td>'+state+',</td><td>Zip:'+zip+'.</td></tr></tbody></table>';
	...
	$("#editbutton").on("click","button",function(){
		...
		// var temp_num = $("#printnumber").find("td").text();
		// var temp_num_type = $("#printnumber").find("th").text();

		// var num_type_html = '<th>Number Type: </th><td><p><input type="text" value="'+temp_num_type+'"></p></td>';
		// var number_html = '<th>Number: </th><td><p><input type="text" value="'+temp_num_type+'"></p></td>';

		// $("#printnumber").children().children().children().eq(0).html(num_html);
		// alert(temp_num_type);
	...
	$("#add_update").on("click","button",function(){
		...
		$.ajax({
			url: "update.php",
			method: "POST",
			data: {index0:contact_id, index1:changed_name, index2:all_nums, index3:all_address, index4:all_dates, index5:number_length, index6:address_length, index7:dates_length},
	...
	$("#addbtn").click(function(){
		...
		var add_name = '<tr id="printname"><td><img src="images/icon.png" class="img-circle" alt="contact img" width="70" height="70"></td><td><p><input type="text"></p><p><input type="text"></p><p><input type="text"></p></td></tr>';
		...
		var add_buttons = '<tr id="add_address"><td colspan="2" style="text-align: center;"><button id="add_addr" type="button" class="btn btn-link">Add Address</button></td></tr><tr id="add_number"><td colspan="2" style="text-align: center;"><button id="add_num" type="button" class="btn btn-link">Add Number</button></td></tr><tr id="add_imp_dates"><td colspan="2" style="text-align: center;"><button id="add_date" type="button" class="btn btn-link">Add Date</button></td></tr><tr id="add_update"><td colspan="2" style="text-align: center;"><button type="button" id="add_contact" class="btn btn-primary">Add Contact</button></td></tr>';
		...
		// alert(all_dates);
			...
			$.ajax({
				url: "insert.php",
				method: "POST",
				data: {index1:changed_name, index2:all_nums, index3:all_address, index4:all_dates, index5:number_length, index6:address_length, index7:dates_length},
